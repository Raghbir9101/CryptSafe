...
export default function SharePage() {
  ...
  const handleAddSharedUser = async (newSharedUser: SharedWithInterface) => {
    ...
    const userExists = table.sharedWith?.some(
      (user) => user.email === newSharedUser.email
    );
    if (userExists) {
    ...
    try {
      ...
      setTable(
        decryptObjectValues(res.data.table, import.meta.env.VITE_GOOGLE_API)
      );
  ...
  const handleUpdateSharedUser = async (
    email: string,
    updatedUser: SharedWithInterface
  ) => {
    if (!table) return;
