...
function EditToolbar(props: GridSlotProps['toolbar']) {
  ...
  const handleClick = () => {
    const id = randomId();
    // Create an empty row with all fields from the table
    const emptyData = tableFields.reduce((acc: any, field: any) => {
      acc[field.name] = '';
      return acc;
    }, {});

    // Create a flattened row with both nested and flattened data structure
    const emptyRow = {
      id,
      isNew: true,
      data: emptyData,
      // Add flattened properties for the DataGrid
      ...Object.entries(emptyData).reduce((acc: any, [key, value]) => {
        acc[`data.${key}`] = value;
        return acc;
      }, {})
    };
...
export default function FullFeaturedCrudGrid() {
  ...
  const processRowUpdate = async (newRow: GridRowModel) => {
    try {
      ...
      if (newRow.isNew) {
        ...
        // Create a flattened version of the saved row with both nested and flat properties
        const savedRow = {
          id: response.data.row._id,
          isNew: false,
          data: processedData,
        ...
        // Create a flattened version of the edited row
        const editedRow = {
          ...newRow,
          isNew: false,
          data: processedData,
  ...
  async function getTable() {
    ...
    api.get(`/tables/rows/${id}`).then((res) => {
      console.log('Fetched rows:', res.data);
      // Flatten the nested data structure for the DataGrid
      const rowsWithFlattenedData = res.data.map((row: any) => {
        ...
        return {
          ...
          data: rowData,
          // Add flattened properties for the DataGrid
          ...Object.entries(rowData).reduce((acc: any, [key, value]) => {
            acc[`data.${key}`] = value;
            return acc;
          }, {})
        };
      ...
      setFetchedRows(rowsWithFlattenedData);
      setRows(rowsWithFlattenedData);
    });
  }
  React.useEffect(() => {
    getTable();
